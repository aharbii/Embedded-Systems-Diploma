...
void bubble_sort(int *array_ptr, int size)
{
    ...
    for (int i = 1; (i < size) && (is_not_sorted); i++)
    {
        ...
        for (int j = 0; j < (size - i); j++)
        {
            ...
            {
                swap_int(&array_ptr[j], &array_ptr[j + 1]);
                is_not_sorted = 1;
            }
        }
    }
...
void selection_sort(int *array_ptr, int size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        swap_int(&array_ptr[size - i - 1], &array_ptr[max_idx]);
    }
}
...
void partition(int *array_ptr, int first, int last)
{
    ...
    while (i < j)
    {
        ...
        {
            swap_int(&array_ptr[i], &array_ptr[j]); // Swapping only if A[i] is greater than A[j]
        }
    }
    if (array_ptr[pivot] > array_ptr[j])
    {
        swap_int(&array_ptr[pivot], &array_ptr[j]); // Setting pivot to its sorted position
...
int partition_second(int *array_ptr, int l, int h)
{
    ...
    swap_int(&array_ptr[l], &array_ptr[j]);
    return j;
}
...
void shell_sort(int *array_ptr, int size)
{
    ...
    while (gab > 0)
    {
        ...
        while (j < size)
        {
            if (array_ptr[i] > array_ptr[j])
            {
                swap_int(&array_ptr[i], &array_ptr[j]); // Insert j element
            }
            ...
            while (new_i >= 0)
            {
                ...
                {
                    swap_int(&array_ptr[new_i], &array_ptr[new_j]);
                }
                new_j = new_i;
                new_i -= gab;
            }
...
void shell_sort_two(int *array_ptr, int size)
{
    for (int gap = size / 2; gap > 0; gap /= 2)
    {
        for (int i = gap; i < size; i++)
        {
            ...
            {
                swap_int(&array_ptr[j], &array_ptr[j + gap]);
            }
        }
    }
}
...
void heap_insert(MaxHeap *h, int key)
{
    ...
    {
        swap_int(&h->data[i - 1], &h->data[(i / 2) - 1]);
        i = i / 2;
    }
}
...
int heap_delete(MaxHeap *h)
{
    ...
    while (j < h->size)
    {
        ...
        {
            swap_int(&h->data[i - 1], &h->data[j - 1]);
            i = j;
            j = i * 2;
        }
        else
